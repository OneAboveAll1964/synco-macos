...
enum ControlMessageFixtures {
    ...
    static let all: [ControlMessage] = [
        ...
        .remoteStart(RemoteStartMessage(maxWidth: 1600, maxHeight: 2560, fps: 60, input: true)),
        .remoteAccept(RemoteAcceptMessage(width: 1600, height: 1000, fps: 60, input: true)),
        .remoteReject(RemoteRejectMessage(reason: .screenPermission)),
        .remoteStop(RemoteStopMessage()),
        .remoteInput(RemoteInputMessage(events: [RemoteInputEvent(kind: "pa", x: 0.5, y: 0.5)])),
        .ping(ping),
        .pong(pong),
        .clip(clip),
