...
public enum HidKeyMap {
    ...
    public static func virtualKey(forUsage usage: Int) -> CGKeyCode? {
        guard let code = table[usage] else { return nil }
        return CGKeyCode(code)
    }

    public static func flags(forModifiers mods: Int) -> CGEventFlags {
        var flags: CGEventFlags = []
        if mods & 0x01 != 0 { flags.insert(.maskShift) }
        if mods & 0x02 != 0 { flags.insert(.maskControl) }
        if mods & 0x04 != 0 { flags.insert(.maskAlternate) }
        if mods & 0x08 != 0 { flags.insert(.maskCommand) }
        return flags
    }
